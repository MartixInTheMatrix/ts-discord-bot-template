    ...
    registerInteractions(guild?:any) {
        ...
        if(!guild){
            this.guilds.cache.forEach(g=>{
                ...
                .catch(()=>console.error(`Error while registering application commands in ${g.name} guild`));
            })
           
        }else{
                rest.put(Routes.applicationGuildCommands(this.user.id, guild.id), { body: commands })
                .then(() => console.log('Successfully registered application commands.'))
                .catch(()=>console.error(`Error while registering application commands in ${(this.guilds.cache.get(guild.id) as discord.Guild).name} guild`));
        }
